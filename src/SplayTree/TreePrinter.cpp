...
using opt_node = std::optional<std::reference_wrapper<Node<int>>>;
...
static auto left(opt_node node) -> opt_node {
    return node ? node->get().left() : std::nullopt;
}

static auto right(opt_node node) -> opt_node {
    return node ? node->get().right() : std::nullopt;
}

// Find the smallest node of a tree
auto find_min(opt_node node) -> opt_node {
    if (!node) {
        return std::nullopt;
    } else if (!left(node)) {
        return node;
...
auto find_max(opt_node node) -> opt_node {
    if (!node) {
        return std::nullopt;
    } else if (!right(node)) {
        return node;
...
template<typename T>
auto find(const T& elem, opt_node node) -> opt_node {
    if (!node) {
        return std::nullopt;
    }
...
template<typename T>
std::unique_ptr<asciinode> build_ascii_tree_recursive(std::optional<Node<int>&> root) {
    std::unique_ptr<asciinode> node;

    if (!root) return node;

    node = std::make_unique<asciinode>();
